...
const EditDeleteEvent = ({ props }: { props: TEvent }) => {
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => onOpenDeleteDialog(`events/${props?._id}`)}>Delete event</DropdownMenuItem>
            </DropdownMenuContent>
        </DropdownMenu>
    )
}
...
export const columns: ColumnDef<TEvent>[] = [
        ...
        cell: ({ row }) => (
            row.getValue("image") ?
                <div className="w-12 h-12">
                    <img className="h-full w-full rounded-md" src={row.getValue("image")} alt={`Event photo`} />
                </div> :
    ...
    },
    {
        ...
        header: "Action",
        cell: ({ row }) => {
            if (row.original.title) return <EditDeleteEvent props={row.original} />
...
export function EventsTable({ data, isLoading }: { data: TEvent[], isLoading: boolean }) {
    ...
    const isEventsExist = isLoading || table.getRowModel().rows?.length && table.getRowModel().rows[0].original.title;
